...
def main():
    ...
    print(f"=== Motordrome Clean Orbit & Apron Climb ===")

    run_dir = make_run_dir(build_run_id("motordrome", "clean_orbit"))
    out_video = Path(run_dir) / "renders" / "motordrome_clean_orbit.mp4"
    ...
    while step < 1600:
        ...
        # Step 0-400: r = 0.75m on flat floor
        # Step 400-1000: r: 0.75 -> 1.42m up 45-deg apron ramp
        # Step 1000+: r = 1.45m on vertical wall
        if step < 400:
            target_r = 0.75
        ...
        # Lookahead along circumference (+theta) with radial tracking
        lookahead = 0.30
        d_th = lookahead / max(target_r, 0.3)
        th_cmd = theta + d_th
        p_target = np.array([target_r * np.cos(th_cmd), target_r * np.sin(th_cmd)])

        d_vec = p_target - pos[:2]
        d_vec /= max(np.linalg.norm(d_vec), 1e-6)
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_clean_orbit.mp4")
    import shutil
