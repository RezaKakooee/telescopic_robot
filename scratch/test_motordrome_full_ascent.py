...
def main():
    ...
    print(f"=== Motordrome Full Wall of Death Ascent & Vertical Spiral ===")

    run_dir = make_run_dir(build_run_id("motordrome", "full_ascent"))
    out_video = Path(run_dir) / "renders" / "motordrome_full_ascent.mp4"
    ...
    while step < 1600:
        ...
        if r < 1.55:
            # Phase 1: Flat floor circular spin-up (accelerates to > 3 m/s)
            phase_label = "Floor Spin-Up (Torque)"
            d_traj = theta_hat + 0.05 * r_hat
        ...
        elif r < 2.32 and z < 0.85:
            # Phase 2: 45° Banked Apron Ramp Climb (helical ascent onto wall)
            phase_label = "45° Apron Ramp Climb"
            n_bank = (-r_hat + z_hat) / np.sqrt(2.0)
            u_upslope = (r_hat + z_hat) / np.sqrt(2.0)

            # Helical climb heading on the 45-deg bank
            d_traj = np.cos(np.radians(18.0)) * theta_hat + np.sin(np.radians(18.0)) * u_upslope
            ...
            # Push into wall (+r_hat) + rearward (-theta_hat) + downward (-z_hat)
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_full_ascent.mp4")
    import shutil
