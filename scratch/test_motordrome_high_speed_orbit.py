...
def main():
    ...
    # Spawn at base on the flat floor
    ...
    print(f"=== Motordrome High-Speed Circular Orbit & Apron Climb ===")

    run_dir = make_run_dir(build_run_id("motordrome", "high_speed_orbit"))
    out_video = Path(run_dir) / "renders" / "motordrome_high_speed_orbit.mp4"
    ...
    while step < 1600:
        ...
        # Target radius progression:
        # Phase 1 (Floor spin-up): r = 0.90m
        # Phase 2 (Apron climb): r: 0.90 -> 1.85m
        # Phase 3 (Wall orbit): r = 1.88m
        if step < 300:
            target_r = 0.90
            ...
            prog = (step - 300) / 600.0
            target_r = 0.90 + prog * (wall_r - 0.15 - 0.90)
            phase_label = "45° Banked Apron Climb"
            climb_pitch = np.radians(14.0)
        else:
            target_r = wall_r - 0.14
            phase_label = "Vertical Wall of Death"
            climb_pitch = np.radians(18.0)

        # Tangential direction (+theta) with radial steering
        r_err = r - target_r
        d_cmd_xy = theta_hat - 0.50 * r_err * r_hat
        d_cmd_xy /= np.linalg.norm(d_cmd_xy)

        # 3D heading with climb pitch
        d_head_3d = np.cos(climb_pitch) * d_cmd_xy + np.sin(climb_pitch) * z_hat
        d_head_3d /= np.linalg.norm(d_head_3d)

        # Decompose body-frame rods along 3D heading
        u_long = dirs_world @ d_head_3d
        u_lat = dirs_world @ r_hat
        u_z = dirs_world[:, 2]

        # Trailing contact quadrant fires with high power
        rear = np.clip((-u_long - 0.08) / 0.92, 0.0, 1.0)
        down = np.clip(1.0 - abs(u_z + 0.35) / 0.85, 0.0, 1.0) if z < 0.40 else np.clip(1.0 - abs(u_lat - 0.40) / 0.80, 0.0, 1.0)
        wave = np.clip((rear ** 1.1) * down * 3.5, 0.0, 1.0)

        # Leading rods stay tucked
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_high_speed_orbit.mp4")
    import shutil
