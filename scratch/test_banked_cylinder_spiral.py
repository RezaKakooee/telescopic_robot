...
def main():
    ...
    y_target = -cyl_rad + 0.10  # offset 10cm inside cylinder rim for perfect tangent entry
    ...
    print(f"=== Seamless Banked Launch & High-Speed Wall of Death Spiral ===")

    run_dir = make_run_dir(build_run_id("ramp_launch", "banked_spiral"))
    out_video = Path(run_dir) / "renders" / "banked_ramp_cylinder_spiral.mp4"
    ...
    while step < 1500:
        ...
        if step % 4 == 0:
            ...
            cam.distance = 3.40
            cam.elevation = -24.0
            cam.azimuth = 135.0 if not in_cylinder else float(np.degrees(theta)) + 115.0
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/banked_ramp_cylinder_spiral.mp4")
    import shutil
