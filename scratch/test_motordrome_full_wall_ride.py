...
def main():
    ...
    print(f"=== Motordrome Wall of Death Full Wall Ride ===")

    run_dir = make_run_dir(build_run_id("motordrome", "full_wall_ride"))
    out_video = Path(run_dir) / "renders" / "motordrome_full_wall_ride.mp4"
    ...
    while step < 1600:
        ...
        if r < 0.95 and z < 0.26:
            # 1. Floor Spin-Up: Drive CCW along circumference
            phase_label = "Floor Spin-Up (Torque)"
            u_traj = theta_hat + 0.10 * r_hat
            u_traj /= np.linalg.norm(u_traj)
            # Rear-downward push vector
            ...
            # 2. 45° Banked Apron Climb: Push against the 45-degree slope
            phase_label = "45° Banked Apron Climb"
            # Bank normal (inward & upward)
            n_bank = (-r_hat + z_hat) / np.sqrt(2.0)
            u_bank_up = (r_hat + z_hat) / np.sqrt(2.0)
            u_traj = np.cos(np.radians(18.0)) * theta_hat + np.sin(np.radians(18.0)) * u_bank_up
            u_push = -u_traj - 0.30 * n_bank
            u_push /= np.linalg.norm(u_push)
            ...
            # 3. Vertical Wall of Death Spiral: Push into wall (+r) + rearward (-theta) + downward (-z)
            phase_label = "Vertical Wall of Death Spiral"
            u_push = 0.60 * r_hat - 0.50 * theta_hat - 0.62 * z_hat
            u_push /= np.linalg.norm(u_push)
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_full_wall_ride.mp4")
    import shutil
