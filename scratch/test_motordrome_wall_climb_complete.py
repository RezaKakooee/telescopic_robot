...
def main():
    ...
    print(f"=== Motordrome Complete Wall of Death Climb ===")

    run_dir = make_run_dir(build_run_id("motordrome", "wall_climb_complete"))
    out_video = Path(run_dir) / "renders" / "motordrome_wall_climb_complete.mp4"
    ...
    while step < 1600:
        ...
        if spd > 0.30:
            ...
            d_traj = 0.60 * v_dir + 0.40 * theta_hat
        else:
            d_traj = theta_hat
        ...
        if r < 1.55 and z < 0.28:
            ...
            d_climb = np.cos(np.radians(14.0)) * d_traj + np.sin(np.radians(14.0)) * u_upslope

            # High-traction push against the bank
            u_push = -0.80 * d_climb - 0.60 * n_bank
            u_push /= np.linalg.norm(u_push)
            p_push = dirs_world @ u_push

            wave = np.clip((p_push - 0.05) / 0.65, 0.0, 1.0) * 1.30
            ...
            wave = np.clip((p_push - 0.05) / 0.70, 0.0, 1.0) * 1.40
            wave = np.clip(wave, 0.0, 1.0)
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_wall_climb_complete.mp4")
    import shutil
