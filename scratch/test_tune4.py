"""Test continuous non-stop rolling with stance support."""
import os
...
def run_continuous4(gain=3.6, ky=1.8, max_steps=400):
    ...
    for step in range(1, max_steps + 1):
        ...
        turn_angle = float(np.clip(-ky * y_now, -0.25, 0.25))
        ...
        # Flank push wave
        rear_flank = (u_long < 0.0) & (np.abs(u_lat) > min_lat) & (u_z < 0.25)
        rear_w = np.clip((-u_long) / 0.85, 0.0, 1.0) ** 0.85
        flank_w = np.clip((np.abs(u_lat) - min_lat) / 0.22, 0.0, 1.0)
        down_w = np.clip(1.0 - np.abs(u_z + 0.35) / 0.80, 0.0, 1.0)

        # Continuous push + active rolling torque
        push = rear_w * flank_w * down_w * gain
        stance = flank_w * down_w * 0.20 * np.clip((-u_long + 0.10) / 0.50, 0.0, 1.0)

        wave = np.clip(push + stance, 0.0, 1.0)
