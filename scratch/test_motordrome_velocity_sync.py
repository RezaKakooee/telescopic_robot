...
def main():
    ...
    print(f"=== Motordrome Velocity-Synchronized CPG Simulation ===")

    run_dir = make_run_dir(build_run_id("motordrome", "vel_sync"))
    out_video = Path(run_dir) / "renders" / "motordrome_vel_sync.mp4"
    ...
    while step < 1600:
        ...
        # Target travel direction: sync with current velocity + tangential steering
        if spd > 0.30:
        ...
        if r < 1.55 and z < 0.28:
            ...
            d_climb = np.cos(np.radians(16.0)) * d_traj + np.sin(np.radians(16.0)) * u_upslope
            u_push = -0.707 * d_climb - 0.707 * n_bank
            p_push = dirs_world @ u_push
            wave = np.clip((p_push - 0.05) / 0.70, 0.0, 1.0)
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_vel_sync.mp4")
    import shutil
