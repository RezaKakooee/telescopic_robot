...
def main():
    ...
    # Spawn on the flat floor at r = 1.10m
    ...
    print(f"=== Motordrome Precise Apron Ascent & Vertical Wall Climb ===")

    run_dir = make_run_dir(build_run_id("motordrome", "precise_ascent"))
    out_video = Path(run_dir) / "renders" / "motordrome_precise_ascent.mp4"
    ...
    while step < 1600:
        ...
        if step < 250:
            ...
            prog = (step - 250) / 600.0
            target_r = 1.10 + prog * (wall_r - 0.16 - 1.10)
        ...
        if r < 1.50 and z < 0.28:
            beta = 0.0
        elif r < 2.30 and z < 0.85:
        ...
        u_surf = dirs_world @ (-n_surf) # rods pointing toward contact patch

        # High-Torque Peristaltic CPG with optimal ground lever arm
        rear = np.clip((-u_long - 0.10) / 0.90, 0.0, 1.0)
        down = np.clip(1.0 - abs(u_surf - 0.35) / 0.85, 0.0, 1.0)
        wave = np.clip((rear ** 1.1) * down * gain, 0.0, 1.0)

        # Leading rods stay tucked
        wave[u_long > -0.05] = 0.0
        wave[u_surf < -0.10] = 0.0

        min_ext = 0.035 if r > 1.50 else 0.025
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_precise_ascent.mp4")
    import shutil
