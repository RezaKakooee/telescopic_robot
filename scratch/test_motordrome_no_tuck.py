...
def main():
    ...
    print(f"=== Motordrome Continuous High-Torque Multi-Lap Orbit ===")

    run_dir = make_run_dir(build_run_id("motordrome", "no_tuck"))
    out_video = Path(run_dir) / "renders" / "motordrome_no_tuck.mp4"
    ...
    while step < 1600:
        ...
        # Step 400-1000: ride up 45-deg apron ramp (r: 1.00 -> 2.20m, z: 0 -> 0.8m)
        # Step 1000+: vertical wooden wall orbit (r = 2.25m, z > 1.0m)
        if step < 400:
            target_r = 1.00
        ...
        # Wide continuous rear push wave (no restrictive lateral tuck!)
        rear = np.clip((-u_long - 0.05) / 0.95, 0.0, 1.0)
        down = np.clip(1.0 - abs(u_norm - 0.50) / 0.75, 0.0, 1.0)
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_no_tuck.mp4")
    import shutil
