...
def main():
    ...
    # Spawn on the flat floor at r = 0.90m (plenty of clearance to apron ramp at r = 1.60m)
    ...
    print(f"=== Motordrome Fast Spin-Up & Wall Riding ===")

    run_dir = make_run_dir(build_run_id("motordrome", "fast_spin"))
    out_video = Path(run_dir) / "renders" / "motordrome_fast_spin.mp4"
    ...
    while step < 1600:
        ...
        # Step 0-400: spin-up on floor (r = 1.00m)
        # Step 400-1000: expand onto 45-deg apron ramp (r: 1.00 -> 2.20m)
        # Step 1000+: vertical wooden wall orbit (r = 2.25m)
        if step < 400:
            target_r = 1.00
        ...
        r_err = r - target_r
        d_cmd_xy = theta_hat - 0.40 * r_err * r_hat
        ...
        wave = np.clip((rear ** 1.1) * down * 3.6, 0.0, 1.0)
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_fast_spin.mp4")
    import shutil
