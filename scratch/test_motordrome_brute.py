...
def main():
    ...
    print(f"=== Motordrome Brute Force Climb ===")

    run_dir = make_run_dir(build_run_id("motordrome", "brute"))
    out_video = Path(run_dir) / "renders" / "motordrome_brute.mp4"
    ...
    while step < 1800:
        ...
        x = float(pos[0])
        y = float(pos[1])
        spd = float(np.linalg.norm(vel[:3])) # 3D speed
        max_z = max(max_z, z)
        ...
        if spd < 4.2 and r < 1.55:
            d_traj = d_traj - 0.2 * r_hat # Steer inward
            d_traj /= np.linalg.norm(d_traj)
        ...
        if r < 1.55 and z < 0.28:
            ...
            # Simple brute force: push backward along velocity and into the bank!
            ...
            wave = np.clip(wave, 0.0, 1.0)
            # DO NOT tuck rods if they are pushing!
            # Only tuck strictly forward-pointing rods
        ...
        if step % 4 == 0:
            ...
            ann_frame = annotate(
                frame,
                "Motordrome Brute Force Simulation",
                [
                    f"Phase: {phase_label}",
