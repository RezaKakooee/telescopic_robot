"""Render 4 side camera views at 30 degrees and 2x2 Quad multi-view video."""
import os
...
renders_dir = Path("/home/azureuser/telescopic_robot/storage_local/20260823_2254__side_cameras_suite")
...
def main():
    ...
    print("Rendering 4 Side 30-degree Cameras & 2x2 Quad Multi-View Video...", flush=True)

    while not done and step < 400:
        action, _ = model.predict(obs, deterministic=True)
        obs, reward, dones, infos = vec_env.step(action)

        # Render 4 side cameras at 30 degrees
        img_fr = raw_env.render(camera_name="side_front_right_30deg")
        img_fl = raw_env.render(camera_name="side_front_left_30deg")
        img_rr = raw_env.render(camera_name="side_rear_right_30deg")
        img_rl = raw_env.render(camera_name="side_rear_left_30deg")

        # 2x2 Quad Multi-View Grid
        ...
        frames_fr.append(img_fr)
        frames_fl.append(img_fl)
        frames_rr.append(img_rr)
        frames_rl.append(img_rl)

        done = dones[0]
        step += 1

    out_quad = renders_dir / "quad_side_30deg_multiview.mp4"
    out_fr = renders_dir / "side_front_right_30deg.mp4"
    out_fl = renders_dir / "side_front_left_30deg.mp4"
    out_rr = renders_dir / "side_rear_right_30deg.mp4"
    out_rl = renders_dir / "side_rear_left_30deg.mp4"

    imageio.mimsave(str(out_quad), frames_quad, fps=30)
    imageio.mimsave(str(out_fr), frames_fr, fps=30)
    imageio.mimsave(str(out_fl), frames_fl, fps=30)
    imageio.mimsave(str(out_rr), frames_rr, fps=30)
    imageio.mimsave(str(out_rl), frames_rl, fps=30)

    thumb_quad = scratch_dir / "quad_side_30deg_thumb.png"
    thumb_fr = scratch_dir / "side_front_right_30deg_thumb.png"
    thumb_fl = scratch_dir / "side_front_left_30deg_thumb.png"
    thumb_rr = scratch_dir / "side_rear_right_30deg_thumb.png"
    thumb_rl = scratch_dir / "side_rear_left_30deg_thumb.png"

    mid = len(frames_quad) // 2
    imageio.imwrite(str(thumb_quad), frames_quad[mid])
    imageio.imwrite(str(thumb_fr), frames_fr[mid])
    imageio.imwrite(str(thumb_fl), frames_fl[mid])
    imageio.imwrite(str(thumb_rr), frames_rr[mid])
    imageio.imwrite(str(thumb_rl), frames_rl[mid])

    print(f"\nAll 4 Side 30-degree Cameras & Quad Multi-View Rendered successfully!")
