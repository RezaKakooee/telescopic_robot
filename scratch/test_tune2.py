"""Test continuous non-stop straddle drive."""
import os
...
def run_continuous_straddle(gain=3.2, ky=2.4, min_lat=0.12, max_steps=450):
    ...
    env.data.qpos[0] = 0.0
    env.data.qpos[1] = 0.0
    env.data.qpos[2] = box_height + 0.20
    env.data.qvel[:] = 0
    mujoco.mj_forward(env.model, env.data)

    for step in range(1, max_steps + 1):
        y_now = float(env.data.qpos[1])
        turn_angle = float(np.clip(-ky * y_now, -0.35, 0.35))
        ...
        # Flank push wave
        rear_flank = (u_long < -0.04) & (np.abs(u_lat) > min_lat) & (u_z < 0.25)
        rear_w = np.clip((-u_long) / 0.85, 0.0, 1.0) ** 0.95
        flank_w = np.clip((np.abs(u_lat) - min_lat) / 0.30, 0.0, 1.0)
        down_w = np.clip(1.0 - np.abs(u_z + 0.25) / 0.80, 0.0, 1.0)

        # Lateral flank differential to help centering
        side_boost = np.where(u_lat * y_now > 0, 0.85, 1.15)

        wave = np.clip(rear_w * flank_w * down_w * gain * side_boost, 0.0, 1.0)
        targets[rear_flank] = env.max_extend * wave[rear_flank]

        # Central void tuck
        central_void = (np.abs(u_lat) < min_lat + 0.02) & (u_z < 0.15)
