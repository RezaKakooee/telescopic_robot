    ...
    for step in range(int(5.0 * 100)):
        ...
        if touching_wall:
            if contact_steps == 0:
                ...
                print(f"  --> HIT WALL at t={step/100:.2f}s: height z={pos[2]:.2f}m, speed v={v_hit_wall:.2f}m/s (along wall: {np.dot(vel, t):.2f}, into wall: {np.dot(vel, n):.2f})")
            contact_steps += 1
        ...
        # Running Hurdle State Machine
        if phase == "approach":
            head = t + np.tan(np.radians(24.0)) * n
            ...
            if along >= 5.5 and wall_dist <= launch_gap:
                phase = "launch"
                print(f"  t={step/100:.2f}s RUNNING LAUNCH: speed={along:.2f}m/s, gap={wall_dist:.2f}m, vx={vel[0]:.2f}, vy={vel[1]:.2f}")

        elif phase == "launch":
            # Running punch: rear-downward rods punch floor while forward-inward momentum carries it up
            ...
            wave[u_z > 0.05] = 0.0  # Only ground-contacting rods push
            targets = (0.025 + (env.max_extend - 0.025) * wave).astype(np.float32)

            if not on_ground and pos[2] > 0.35:
                phase = "fly"
                print(f"  t={step/100:.2f}s FLY: apex climbing, pos[2]={pos[2]:.2f}m, speed={np.linalg.norm(vel):.2f}m/s, vz={vel[2]:.2f}m/s")
            ...
            # High-speed along-wall drive and upward lift
            ...
            targets[lower] = np.clip(targets[lower] + 0.70 * env.max_extend * (-u_z[lower]), 0.0, env.max_extend)
...
if __name__ == "__main__":
    test_hurdle_launch("curved", speed=8.5, launch_gap=1.65, launch_up=5.6, launch_in=3.6)
    test_hurdle_launch("banked", speed=8.5, launch_gap=1.65, launch_up=5.6, launch_in=3.6)
    test_hurdle_launch("flat_multistep", speed=8.5, launch_gap=1.65, launch_up=5.6, launch_in=3.6)
