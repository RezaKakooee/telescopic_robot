...
def test_explosive_jump(mode="curved", speed=7.5, launch_up=6.0, launch_in=3.5, launch_gap=1.55):
    ...
    for step in range(int(5.0 * 100)):
        ...
        # Explosive Launch state machine
        if phase == "approach":
            head = t + np.tan(np.radians(20.0)) * n
            ...
            if along >= 5.5 and wall_dist <= launch_gap:
                phase = "launch"
                print(f"  t={step/100:.2f}s LAUNCH TRIGGERED: pos=({pos[0]:.2f}, {pos[1]:.2f}, {pos[2]:.2f}), speed={along:.2f}m/s, gap={wall_dist:.2f}m")

        elif phase == "launch":
            # Maximum explosive thrust: All downward & away-facing rods extend 100% at max rate
            want = launch_in * n + launch_up * np.array([0.0, 0.0, 1.0])
            want = want / max(float(np.linalg.norm(want)), 1e-9)
            aim = -(dirs_world @ want)
            gain = float(np.clip(max((launch_in - float(np.dot(vel, n))) / 0.60,
                                     (launch_up - float(vel[2])) / 0.60), 0.0, 1.0))
            wave = np.clip((aim - 0.02) / 0.98, 0.0, 1.0) * gain
            wave[u_z > 0.05] = 0.0  # Only downward-directed rods push
            targets = (0.025 + (env.max_extend - 0.025) * wave).astype(np.float32)

            if not on_ground and pos[2] > 0.45:
                phase = "fly"
...
if __name__ == "__main__":
    test_explosive_jump("curved", speed=7.5, launch_up=6.0, launch_in=3.6, launch_gap=1.60)
    test_explosive_jump("banked", speed=7.5, launch_up=6.0, launch_in=3.6, launch_gap=1.60)
    test_explosive_jump("flat_multistep", speed=7.5, launch_up=6.0, launch_in=3.6, launch_gap=1.60)
