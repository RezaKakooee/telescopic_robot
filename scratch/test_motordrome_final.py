...
def main():
    ...
    print(f"=== Motordrome Final Ascent & High Wall Ride ===")

    run_dir = make_run_dir(build_run_id("motordrome", "final"))
    out_video = Path(run_dir) / "renders" / "motordrome_final.mp4"
    ...
    while step < 1800:
        ...
        if r < 1.55 and z < 0.28:
            ...
            d_climb = np.cos(np.radians(22.0)) * d_traj + np.sin(np.radians(22.0)) * u_upslope

            u_push = -np.cos(np.radians(35.0)) * d_climb - np.sin(np.radians(35.0)) * n_bank
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_final.mp4")
    import shutil
