...
def main():
    ...
    print(f"=== Motordrome Smooth Climb ===")

    run_dir = make_run_dir(build_run_id("motordrome", "smooth"))
    out_video = Path(run_dir) / "renders" / "motordrome_smooth.mp4"
    ...
    while step < 1800:
        ...
        if spd > 0.30:
            v_dir = np.array([vel[0] / spd, vel[1] / spd, 0.0])
        else:
            v_dir = theta_hat

        # Steer slightly outward to smoothly enter ramp
        d_traj = 0.70 * v_dir + 0.30 * theta_hat + 0.1 * r_hat
        ...
        if r < 1.55 and z < 0.28:
            phase_label = f"Floor Spin-Up (spd: {spd:.2f})"
            u_push = -0.7 * d_traj - 0.7 * z_hat
            p_push = dirs_world @ u_push
            wave = np.clip((p_push - 0.05) / 0.70, 0.0, 1.0) * 1.5
            wave = np.clip(wave, 0.0, 1.0)
            wave[dirs_world @ d_traj > 0.0] = 0.0
            ...
            phase_label = f"45° Ramp (spd: {spd:.2f})"
            n_bank = (-r_hat + z_hat) / np.sqrt(2.0)
            u_upslope = (r_hat + z_hat) / np.sqrt(2.0)

            # Steer slightly UP the slope to overcome gravity
            d_climb = np.cos(np.radians(25.0)) * d_traj + np.sin(np.radians(25.0)) * u_upslope
            d_climb /= np.linalg.norm(d_climb)

            # Push firmly into the bank and backwards
            u_push = -0.8 * d_climb - 0.6 * n_bank
            u_push /= np.linalg.norm(u_push)
            p_push = dirs_world @ u_push

            wave = np.clip((p_push - 0.05) / 0.65, 0.0, 1.0) * 1.5
            wave = np.clip(wave, 0.0, 1.0)
            wave[dirs_world @ d_climb > 0.1] = 0.0

            targets = np.full(60, 0.015, dtype=np.float32)
            targets[p_push > 0.05] = 0.015 + (env.max_extend - 0.015) * wave[p_push > 0.05]

        else:
            phase_label = f"Vertical Wall (spd: {spd:.2f})"
            n_wall = -r_hat
            d_climb = np.cos(np.radians(10.0)) * d_traj + np.sin(np.radians(10.0)) * z_hat
            d_climb /= np.linalg.norm(d_climb)

            u_push = -0.7 * d_climb - 0.7 * n_wall
            u_push /= np.linalg.norm(u_push)
            p_push = dirs_world @ u_push
        ...
        if step % 4 == 0:
            ...
            ann_frame = annotate(
                frame,
                "Motordrome Smooth Transition Simulation",
                [
                    f"Phase: {phase_label}",
