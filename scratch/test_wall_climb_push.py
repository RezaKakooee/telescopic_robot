...
def main():
    ...
    print(f"=== Motordrome Try to Climb ===")

    run_dir = make_run_dir(build_run_id("motordrome", "try_climb"))
    out_video = Path(run_dir) / "renders" / "motordrome_try_climb.mp4"
    ...
    while step < 1800:
        ...
        if spd > 0.30:
            ...
            d_traj = 0.70 * v_dir + 0.30 * theta_hat
        else:
            d_traj = theta_hat
        ...
        # Transition zone - reach out to ramp before slamming into it
        elif r < 1.65 and z < 0.28:
            phase_label = "Transition to Ramp"
            # Anticipate the ramp which is at 45 degrees
            n_bank = (-r_hat + z_hat) / np.sqrt(2.0)
            u_upslope = (r_hat + z_hat) / np.sqrt(2.0)
            d_climb = np.cos(np.radians(20.0)) * d_traj + np.sin(np.radians(20.0)) * u_upslope

            u_push = -0.7 * d_climb - 0.7 * n_bank
            u_push /= np.linalg.norm(u_push)
            p_push = dirs_world @ u_push

            wave = np.clip((p_push - 0.05) / 0.65, 0.0, 1.0) * 1.5
            wave = np.clip(wave, 0.0, 1.0)
            wave[dirs_world @ d_climb > -0.05] = 0.0

            targets = np.full(60, 0.015, dtype=np.float32)
            targets[p_push > 0.05] = 0.015 + (env.max_extend - 0.015) * wave[p_push > 0.05]

        # On the 45 degree ramp
        elif r < 2.35 and z < 0.85:
            phase_label = "45° Apron Ramp Climb"
            n_bank = (-r_hat + z_hat) / np.sqrt(2.0)
            u_upslope = (r_hat + z_hat) / np.sqrt(2.0)
            d_climb = np.cos(np.radians(30.0)) * d_traj + np.sin(np.radians(30.0)) * u_upslope

            u_push = -0.6 * d_climb - 0.8 * n_bank
            u_push /= np.linalg.norm(u_push)
            p_push = dirs_world @ u_push
        ...
        # On the vertical wall
            ...
            u_push = 0.70 * r_hat - 0.40 * theta_hat - 0.60 * z_hat
            u_push /= np.linalg.norm(u_push)
            p_push = dirs_world @ u_push
