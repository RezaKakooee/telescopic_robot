...
def main():
    ...
    # Spawn on the flat floor at r = 1.20m (plenty of room to spin up to 3 m/s)
    ...
    print(f"=== Motordrome High-Clearance Wall of Death Orbit ===")

    run_dir = make_run_dir(build_run_id("motordrome", "high_clearance"))
    out_video = Path(run_dir) / "renders" / "motordrome_high_clearance.mp4"
    ...
    while step < 1600:
        ...
        u_norm = dirs_world @ (-n_surf) # rods pointing toward contact patch

        # Continuous peristaltic wave with active clearance
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_high_clearance.mp4")
    import shutil
