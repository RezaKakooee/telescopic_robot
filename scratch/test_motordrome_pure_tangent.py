...
def main():
    ...
    print(f"=== Motordrome Pure Tangential Circle Drive ===")

    run_dir = make_run_dir(build_run_id("motordrome", "pure_tangent"))
    out_video = Path(run_dir) / "renders" / "motordrome_pure_tangent.mp4"
    ...
    while step < 1600:
        ...
        # Step 300-800: r: 0.75 -> 1.40m up the 45-deg apron ramp
        # Step 800+: r = 1.45m on the vertical wall
        if step < 300:
            target_r = 0.75
        ...
        # Tangential heading (+theta) with proportional radial correction
        r_err = r - target_r
        # Direction: forward along circle (+theta) + inward/outward correction
        theta_hat = np.array([-np.sin(theta), np.cos(theta)])
        r_hat = np.array([np.cos(theta), np.sin(theta)])
        d_cmd = theta_hat - 1.2 * r_err * r_hat
    ...
    dst = Path("/home/azureuser/.gemini/antigravity-ide/brain/cae66589-5edc-46dd-9306-d193640ffe8c/motordrome_pure_tangent.mp4")
    import shutil
