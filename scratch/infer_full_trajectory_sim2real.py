"""Full Trajectory Sim-to-Real Inference Benchmark.

Runs until the robot autonomously navigates through all maze corridors and reaches the final goal.
Active Stack:
- Full 100% Sim-to-Real Physics (Rubber footpads, motor dynamics, 25ms latency, sensor noise).
- Photorealistic Carbon Gunmetal Robot Theme with Titanium Collars & Black Rubber Feet.
- Complete Multi-Camera Trajectory Video Suite (Bird Chase, Dual View, Triple View, 3D Chase).
"""
...
renders_dir = Path("/home/azureuser/telescopic_robot/storage_local/20260823_2342__full_trajectory_sim2real_suite")
...
class FullTrajectorySim2RealEnv(MujocoSteeringEnv):
...
def main():
    vec_env = DummyVecEnv([lambda: FullTrajectorySim2RealEnv(cfg, scenario=sc, latency_ms=25.0, randomize=False, max_steps=2500)])
    if norm_path.exists():
    ...
    print("Running Full Trajectory Sim-to-Real Autonomous Navigation to Goal...", flush=True)

    # Sample video frame every 2 steps to generate a compact, smooth 30 FPS video of the full run
    frame_interval = 2
    ...
    while not done and step < 2500:
        ...
        if dist_to_goal < 0.45:
            print(f"\n🎯 GOAL REACHED! Successfully navigated into goal cylinder in {step} steps!", flush=True)
            done = True
            break
    ...
    print("\n=========================================================================")
    print("FULL TRAJECTORY SIM-TO-REAL NAVIGATION RESULTS")
    print("=========================================================================")
    print(f"Goal Success: {success} | Final Dist to Goal: {final_dist:.3f} m | Total Steps: {step}")
    print(f"Wall Contacts: {wall_contacts} ({wall_contacts/step*100:.1f}%) | Mean Speed: {avg_speed:.2f} m/s")
    ...
    print("\nEncoding full trajectory video suite...", flush=True)
    out_normal_bird = renders_dir / "full_trajectory_bird_chase.mp4"
    out_normal_dual = renders_dir / "full_trajectory_dual_bird.mp4"
    out_normal_triple = renders_dir / "full_trajectory_triple_composite.mp4"
    out_normal_3d = renders_dir / "full_trajectory_3d_chase.mp4"
    ...
    thumb_dual = scratch_dir / "full_trajectory_dual_thumb.png"
    thumb_triple = scratch_dir / "full_trajectory_triple_thumb.png"
    thumb_bird = scratch_dir / "full_trajectory_bird_thumb.png"
    thumb_goal = scratch_dir / "full_trajectory_goal_thumb.png"
    ...
    print(f"\nAll Full Trajectory Videos Saved to {renders_dir}!")
    vec_env.close()
