"""Test straddle drive with proportional heading centering."""
import os
...
def run_flank_centered(gain=2.8, ky=1.8, min_lat=0.15, max_steps=450):
    ...
    for step in range(1, max_steps + 1):
        ...
        # Steer heading slightly left if y < 0, right if y > 0
        ...
        # Flank push wave
        rear_flank = (u_long < -0.05) & (np.abs(u_lat) > min_lat) & (u_z < 0.20)
        rear_w = np.clip((-u_long) / 0.85, 0.0, 1.0)
        flank_w = np.clip((np.abs(u_lat) - min_lat) / 0.35, 0.0, 1.0)
        down_w = np.clip(1.0 - np.abs(u_z + 0.25) / 0.75, 0.0, 1.0)
...
print("Testing centered flank drive...")
run_flank_centered(gain=2.8, ky=2.2)
