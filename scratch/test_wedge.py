"""Test wedge-free flank peristalsis across dual boxes."""
import os
...
def run_wedge_free(gain=3.0, ky=2.4, max_steps=500):
    ...
    for step in range(1, max_steps + 1):
        ...
        turn_angle = float(np.clip(-ky * y_now, -0.32, 0.32))
        ...
        # Flank push wave: only push with rear flank rods that don't wedge downward
        rear_w = np.clip((-u_long - 0.05) / 0.85, 0.0, 1.0) ** 0.95
        flank_w = np.clip((np.abs(u_lat) - min_lat) / 0.28, 0.0, 1.0)
        down_w = np.clip(1.0 - np.abs(u_z + 0.18) / 0.50, 0.0, 1.0)

        side_bias = np.where(u_lat * y_now > 0, 0.80, 1.20)
        ...
        # 2. Never extend steep downward rods that could wedge into corners
        wave[u_z < -0.55] = 0.0

        # 3. Never extend leading rods
        ...
        if env.data.qpos[0] >= 5.0:
            print(f"🏁 Reached finish line at step {step} ({step*0.01:.2f}s)!")
            break
