...
`skills/mid_level/stair_climbing.py` is the stateless dispatcher. The course-level state machine
is in `demos/stairs/runner.py`; it reads geometry from
...
```bash
MUJOCO_GL=egl PYTHONPATH=. python demos/stairs/runner.py video=false
MUJOCO_GL=egl PYTHONPATH=. python demos/stairs/runner.py
