...
One folder per demo. Each holds everything about showing one skill:
`demo.yaml`, and `runner.py` when the control flow is the point.

    demos/gap/demo.yaml        scenario, skill, expectations, video panes
    demos/wall_run/runner.py   the ten-phase machine

`scripts/run_demo.py` runs them. A demo with a runner is executed by it; a
demo without one is executed declaratively by `radial_sphere/demo.py`.
